...
exports.createUser = async (req, res) => {
  ...
  // Super Admin (99): usuario master de Crumi, nunca exige ni usa tenant. Otros roles requieren empresa.
  let effectiveRoleId = role_id;
  let roleIdNum = parseInt(role_id, 10);
  let isSuperAdminRole = roleIdNum === 99 || role_id === 99 || role_id === '99';
  let tenant_id = isSuperAdminRole ? null : (tenantIdFromBody || req.user?.tenant_id);

  // Respaldo: si quien crea es Super Admin y no envió empresa, asumir que crea otro Super Admin
  const currentUserIsSuperAdmin = req.user && (parseInt(req.user.role_id, 10) === 99);
  if (currentUserIsSuperAdmin && (tenantIdFromBody === undefined || tenantIdFromBody === null || tenantIdFromBody === '')) {
    if (role_id == 99 || role_id === '99' || role_id === undefined || role_id === null || role_id === '') {
      isSuperAdminRole = true;
      tenant_id = null;
      effectiveRoleId = (role_id != null && role_id !== '') ? role_id : 99;
      roleIdNum = 99;
    }
  }

  const isStylist = roleIdNum === 3;
  const isClient  = roleIdNum === 4;

  if (!effectiveRoleId || !first_name || !email || !password) {
    return res.status(400).json({ error: 'Faltan campos obligatorios.' });
  }
  if (!isSuperAdminRole && !tenant_id) {
    return res.status(400).json({ error: 'Falta tenant (empresa) para este rol.' });
  }
  ...
  try {
    ...
    if (isClient) {
      await upsertThirdParty(db, {
        ...
        documentType: 'CC',
        documentNumber: result.rows[0].phone || result.rows[0].email || `USER-${result.rows[0].id}`,
        name: `${result.rows[0].first_name} ${result.rows[0].last_name || ''}`.trim(),
        email: result.rows[0].email,
        phone: result.rows[0].phone,
        status: 'ACTIVO',
        metadata: { role_id: result.rows[0].role_id }
      });
...
/* =========================================================
   ...
   Soporta ?role_id=3 (un solo rol) o ?role_ids=2,3 (varios)
========================================================= */
exports.getAllUsersByTenant = async (req, res) => {
  ...
  if (role_ids) {
    const ids = role_ids.split(',').map(r => parseInt(r.trim(), 10)).filter(n => !isNaN(n));
    if (ids.length > 0) {
      sql += ` AND role_id = ANY($2::int[])`;
      params.push(ids);
    }
  } else if (role_id) {
    sql += ' AND role_id = $2';
...
/* =========================================================
   Listado de Contadores (rol 4) — Solo Super Admin
========================================================= */
exports.getContadores = async (req, res) => {
  const roleId = req.user && (parseInt(req.user.role_id, 10) || req.user.role_id);
  if (roleId !== 99) {
    return res.status(403).json({ error: 'Solo Super Admin puede ver el listado de contadores.' });
  }

  try {
    const r = await db.query(
      `SELECT u.id, u.tenant_id, u.role_id, u.first_name, u.last_name, u.email, u.phone, u.status, u.created_at,
...
/* =========================================================
   Listado de TODOS los usuarios — Solo Super Admin
========================================================= */
exports.getAllUsersForSuperAdmin = async (req, res) => {
  const roleId = req.user && (parseInt(req.user.role_id, 10) || req.user.role_id);
  if (roleId !== 99) {
    return res.status(403).json({ error: 'Solo Super Admin puede ver el listado completo de usuarios.' });
  }
  ...
  try {
    ...
    console.error('Error en getAllUsersForSuperAdmin:', error);
    return res.status(500).json({ error: 'Error interno del servidor' });
  }
};
