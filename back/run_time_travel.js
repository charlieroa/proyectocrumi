...
// ====================== CONFIG ======================
const TIME_TRAVEL_DATE = '2019-05-20'; // Fecha segura dentro del rango 2019
const TIME_TRAVEL_TIME = '12:00:00-05:00';
...
const DIAN_CONFIG = {
    ...
    EMISOR: {
        NIT: process.env.DIAN_NIT || '79401490',
        DV: '2',
        RAZON_SOCIAL: process.env.DIAN_RAZON_SOCIAL || 'LUIS MIGUEL MORENO CARDONA',
        NOMBRE_COMERCIAL: 'LMMC',
        CIUDAD: 'Bogotá', DEPARTAMENTO: 'Bogotá D.C.', CODIGO_MUNICIPIO: '11001', CODIGO_DEPARTAMENTO: '11', TIPO_PERSONA: '2',
...
const run = async () => {
    console.log(`🚀 TIME TRAVEL TO ${TIME_TRAVEL_DATE} (30 FC, 10 NC, 10 ND)...`);
    ...
    const items = [{ description: 'Test Time Travel', quantity: 1, unitPrice: 1000, lineTotal: 1000, lineBase: 1000, tax: 19, taxVal: 190, code: 'T1' }];
    const subtotal = 1000, taxAmount = 190, total = 1190;
    const REF_INVOICE = 'SETP990008001'; // Referencia a la primera factura que enviaremos

    // 1. FACTURAS (30)
    for (let i = 1; i <= 30; i++) {
        const num = (990008000 + i).toString();
        console.log(`\n📄 [${i}/30] FC (2019) ${num}`);
        const data = {
            invoiceNumber: num,
            date: TIME_TRAVEL_DATE, // <--- 2019
            time: TIME_TRAVEL_TIME,
        ...
        if (res.success && res.dianResponse?.IsValid) console.log("   ✅ OK"); else console.log("   ❌ ERR", JSON.stringify(res));
        await new Promise(r => setTimeout(r, 1000));
    }

    // 2. NOTAS CRÉDITO (10)
    for (let i = 1; i <= 10; i++) {
        const num = (990009000 + i).toString();
        console.log(`\n💳 [${i}/10] NC (2019) ${num}`);
        const data = {
            noteNumber: num,
            refInvoiceNumber: REF_INVOICE, // Usamos la primera FC generada como ref
            date: TIME_TRAVEL_DATE, // <--- 2019
            time: TIME_TRAVEL_TIME,
            notes: 'Time Travel Test', items, subtotal, taxAmount, total, software: DIAN_CONFIG.SOFTWARE, emisor: DIAN_CONFIG.EMISOR, client, resolucion: DIAN_CONFIG.RESOLUCION, pin: DIAN_CONFIG.SOFTWARE.PIN, tipoAmbiente: '2', numDocAdquiriente: client.idNumber
        };
    ...
    // 3. NOTAS DÉBITO (10)
    for (let i = 1; i <= 10; i++) {
        const num = (990010000 + i).toString();
        console.log(`\n📈 [${i}/10] ND (2019) ${num}`);
        const data = {
            noteNumber: num,
