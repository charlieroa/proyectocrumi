...
// Config - DATOS CORRECTOS DE CRUMI S.A.S
const TODAY = new Date().toISOString().split('T')[0]; // 2025-12-29
const NOW_TIME = new Date().toTimeString().split(' ')[0] + '-05:00';

console.log(`🗓️ Fecha de envío: ${TODAY} ${NOW_TIME}`);

const DIAN_CONFIG = {
    AMBIENTE: '2',
    SOFTWARE: {
        ID: 'cd7aa0f8-5eb5-49d7-9276-b55a79cfc260', // iacrumi
        PIN: '10225', // PIN de la imagen
    ...
    EMISOR: {
        ...
        DV: '0',  // Calcular DV correcto más adelante si es necesario
        RAZON_SOCIAL: 'CRUMI S.A.S',
        NOMBRE_COMERCIAL: 'CRUMI',
        CIUDAD: 'Bogotá', DEPARTAMENTO: 'Bogotá D.C.', CODIGO_MUNICIPIO: '11001', CODIGO_DEPARTAMENTO: '11', TIPO_PERSONA: '1', // 1 = Persona Jurídica
...
(async () => {
    console.log("🚀 Enviando factura con NIT CORRECTO: 902006720 (CRUMI S.A.S)");

    const num = 'SETP990012001';
    const client = { name: 'CONSUMIDOR FINAL', idNumber: '222222222222', docType: '13', dv: '2' };
    const items = [{ description: 'Test CRUMI', quantity: 1, unitPrice: 1000, lineTotal: 1000, lineBase: 1000, tax: 19, taxVal: 190, code: 'T1' }];
    ...
    const data = {
        ...
        software: DIAN_CONFIG.SOFTWARE, emisor: DIAN_CONFIG.EMISOR, client,
        resolucion: DIAN_CONFIG.RESOLUCION, nitEmisor: DIAN_CONFIG.EMISOR.NIT,
        claveTecnica: DIAN_CONFIG.SOFTWARE.PIN, numDocAdquiriente: client.idNumber,
        tipoAmbiente: '2'
    };

    console.log(`📄 Factura: ${num}`);
    console.log(`   NIT Emisor: ${DIAN_CONFIG.EMISOR.NIT}`);
    console.log(`   Razón Social: ${DIAN_CONFIG.EMISOR.RAZON_SOCIAL}`);
    ...
    if (res.success && res.zipKey) {
        console.log("\n✅ DOCUMENTO RECIBIDO POR DIAN");
        console.log(`   ZipKey: ${res.zipKey}`);
    } else {
        console.log("\n❌ POSIBLE ERROR - Revisar respuesta arriba");
    }
})();
